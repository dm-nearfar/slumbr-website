...
1. **Body-level sky.** `body` carries the gradient (`#1E1B4B` at the top
   ...
   Those children, in DOM paint order, are `body::before` (starfield) and
   `body::after` (crescent moon), both at `z-index: -1`, so stars sit under
   the moon and both under every piece of content on every page. Everything else (the
