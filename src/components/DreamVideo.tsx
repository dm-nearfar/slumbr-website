...
// autoplay. This keeps each clip fully static (no client JS), which is best for
// Core Web Vitals. The markup is built from a known poster path, never user input.
//
// Sources are listed WebM-first (smaller, modern browsers) then MP4 (universal
// fallback, must be encoded +faststart). Both are derived from the poster name.
...
export default function DreamVideo({ poster, label }: DreamVideoProps) {
  ...
  const html =
    `<video class="my-8 h-auto w-full rounded-2xl border border-border bg-surface"` +
